...
class NotHorizontalConstraint(BinaryConstraint):
	def isSatisfied(self, value1, value2):
		import re
		value1 = value1.split('|')
		value2 = value2.split('|')
	...
	def __repr__(self):
	    return 'NotHorizontalConstraint (%s, %s)' % (str(self.var1), str(self.var2))


class NotVerticalConstraint(BinaryConstraint):
	def isSatisfied(self, value1, value2):
		import re
		value1 = value1.split('|')
		value2 = value2.split('|')
	...
	def __repr__(self):
	    return 'NotVerticalConstraint (%s, %s)' % (str(self.var1), str(self.var2))

class NotDiagonalConstraint(BinaryConstraint):
	def isSatisfied(self, value1, value2):
		import re
		value1 = value1.split('|')
		value2 = value2.split('|')
		...
		if abs(value1[0] - value2[0]) == abs(value1[1] - value2[1]):
			return False
		return True
...
def consistent(assignment, csp, var, value):
	...
	for constraint in constraints:
		if constraint.affects(var):
			...
			#solutions = assignment.extractSolution()
			if assignment.isAssigned(other):
				if not constraint.isSatisfied(value, assignment.assignedValues[other]):
				#print "returning False"
					return False

	#get constraints: csp.binaryConstraints
	#find constraints that include the current variable
	#loop through all of the constraints and check constraint.isSatisfied
...
def backtracking(assignment, csp, orderValuesMethod, selectVariablesMethod):
	#if assignment is complete then return assignment
	if assignment.isComplete():
		return assignment

	#var <- SELECT-UNASSIGNED-VARIABLE(csp)
	var = selectVariablesMethod(assignment, csp)
	orderedValues = orderValuesMethod(assignment, csp, var)
	for value in orderedValues:
		if consistent(assignment, csp, var, value):
			assignment.assignedValues[var] = value
			result = recursiveBacktracking(assignment, csp, orderValuesMethod, selectVariablesMethod)
			if not result == None:
				return result
...
def minimumRemainingValuesHeuristic(assignment, csp):
	...
	# loop 1: get the minimum value
	# loop 2: find all the variables that have the minimum value and add them to a list
	# then run the tiebreakers if list is greater than length 1

	for var in domains:
		values = domains[var]
		if len(values) < curMin and not assignment.isAssigned(var):
			curMin = len(values)
			# nextVar = var
			# print "min:" + str(curMin)

	minDict = dict()
	for var in domains:
		if not assignment.isAssigned(var):
			if len(domains[var]) == curMin:
				minDict[var] = domains[var]
	...
	if len(minDict) > 1:
		maxDegree = -1
		for key in keys:
			# print "nextVar is: " + str(nextVar)
			if not assignment.isAssigned(key):

				#varValues = minDict[key]
				counter = 0
				for constraint in csp.binaryConstraints:
					if constraint.affects(key) and not assignment.isAssigned(constraint.otherVariable(key)):
						counter = counter + 1
				if counter > maxDegree:
		...
		# #break tie
		# if len(values) == curMin:
		# 	varCounter = 0
		# 	nextVarCounter = 0
		# 	for constraint in csp.binaryConstraints:
		# 		if constraint.affects(var) and assignment.assignedValues[constraint.otherVariable(var)] == None:
		# 			varCounter = varCounter + 1
		# 		if constraint.affects(nextVar) and assignment.assignedValues[constraint.otherVariable(nextVar)] == None:
		# 			nextVarCounter = nextVarCounter + 1
		# 	if varCounter > nextVarCounter:
		# 		nextVar = var
	if nextVar == None:
		print "nextVar == None"
	return nextVar
...
def leastConstrainingValuesHeuristic(assignment, csp, var):
	...
	newValues = list()
	for value in values:
		constrained = checkConstraints(assignment, csp, var, value)
		newValues.append((constrained, value))

	newValues = sorted(newValues)

	returnValues = list()
	for value in newValues:
		returnValues.append(value[1])

	values = returnValues
	return values

def checkConstraints(assignment, csp, var, value):
	constrained = 0

	for constraint in csp.binaryConstraints:
		if constraint.affects(var):
			for num in assignment.varDomains[constraint.otherVariable(var)]:
				if not constraint.isSatisfied(num, value):
...
def forwardChecking(assignment, csp, var, value):
	...
	for variable in domains:
		#print "domain: " + str(assignment.varDomains[variable])
		for constraint in csp.binaryConstraints:
			if constraint.affects(var) and not assignment.isAssigned(constraint.otherVariable(var)):
				#print "constraint: " + str(constraint)
				for option in assignment.varDomains[constraint.otherVariable(var)]:
					if not constraint.isSatisfied(value, option):
						inferences.add((constraint.otherVariable(var), option))

	for inference in inferences:
		assignment.varDomains[inference[0]].remove(inference[1])
		if len(assignment.varDomains[inference[0]]) == 0:
			#reassign values
			for inference in inferences:
...
def recursiveBacktrackingWithInferences(assignment, csp, orderValuesMethod, selectVariableMethod, inferenceMethod):
	"""Question 4"""
	"""YOUR CODE HERE"""
	return backtrackingWithInferences(assignment, csp, orderValuesMethod, selectVariableMethod, inferenceMethod)

def backtrackingWithInferences(assignment, csp, orderValuesMethod, selectVariablesMethod, inferenceMethod):
	# if assignment is complete then return assignment
	if assignment.isComplete():
		return assignment
	...
	for value in orderedValues:
		if consistent(assignment, csp, var, value):
			...
			if not inferences == None:
				result = recursiveBacktrackingWithInferences(assignment, csp, orderValuesMethod, selectVariablesMethod, inferenceMethod)
				if not result == None:
					return result
				for inference in inferences:
					assignment.varDomains[inference[0]].add(inference[1])
...
def maintainArcConsistency(assignment, csp, var, value):
	...
	for constraint in csp.binaryConstraints:
		if constraint.affects(var) and not assignment.isAssigned(constraint.otherVariable(var)):
			#check if the other variable is assigned before put in the queue
			myQueue.put((var, constraint.otherVariable(var), constraint))

	while not myQueue.empty():
		constraintPop = myQueue.get()
		revisions = revise(assignment, csp, constraintPop[0], constraintPop[1], constraintPop[2])
		if revisions == None:
			for inference in inferences:
				assignment.varDomains[inference[0]].add(inference[1])
			return None
		...
		if len(revisions) > 0:
			...
			otherVar = constraintPop[1]
			for constraint in csp.binaryConstraints:
				if constraint.affects(otherVar) and not assignment.isAssigned(constraint.otherVariable(otherVar)):
					#check if the other variable is assigned before put in the queue
					if not constraint.otherVariable(otherVar) is var:
						myQueue.put((otherVar, constraint.otherVariable(otherVar), constraint))
	return inferences
...
def AC3(assignment, csp):
	...
	for constraint in csp.binaryConstraints:
		...
		myQueue.put((var1, constraint.otherVariable(var1), constraint))

		var2 = constraint.var2
		myQueue.put((var2, constraint.otherVariable(var2), constraint))
	...
	while not myQueue.empty():
		...
		if len(revisions) > 0:
			...
			for constraint in csp.binaryConstraints:
				if constraint.affects(otherVar) and not assignment.isAssigned(constraint.otherVariable(otherVar)):
					#check if the other variable is assigned before put in the queue
					if constraint.otherVariable(otherVar) != constraintPop[0]:
						myQueue.put((otherVar, constraint.otherVariable(otherVar), constraint))
	return assignment
