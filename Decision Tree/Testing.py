from DataInterface import getDummyDataset2,getDummyDataset1,getConnect4Dataset, getCarDataset, getBalanceDataset
...
def testBalance(setFunc = setEntropy, infoFunc = infoGain):
    """Correct classification averate rate is about 0.95"""
    examples,attrValues,labelName,labelValues = getBalanceDataset()
    print 'Testing Car dataset. Number of examples %d.'%len(examples)
    tree = makeTree(examples, attrValues, labelName, setFunc, infoFunc)
...
if __name__ == "__main__":
    # getAverageClassificaionRate(getDummyDataset1())
    # getAverageClassificaionRate(getDummyDataset2())
