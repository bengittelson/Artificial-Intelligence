...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        # print "noisyDistance: " + str(noisyDistance)
        # print "emissionModel: " + str(emissionModel) 
        # print "pacmanPosition: " + str(pacmanPosition)
        "*** YOUR CODE HERE ***"
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        #basic idea: sum up probabilities in a for loop 


        #make an all possible and update it at the end (set it to self. beliefs) 
        #add values in each cell, but multiply each value by the probability that Pacman is in the corresponding position
        ...
        for position in self.legalPositions:
            ...
            #iterate over new Pos Dist:
            for newPos, prob in newPosDist.items():
                allPossible[newPos] = allPossible[newPos] + newPosDist[newPos] * self.beliefs[position]
        allPossible.normalize()
        self.beliefs = allPossible
        "*** YOUR CODE HERE ***"
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        # partPerPos = self.numParticles / len(self.legalPositions)
        #
        #     for j in range(0, partPerPos):
        #         particles.append(self.legalPositions[i])
        # return particles
    ...
    def observe(self, observation, gameState):
        ...
        #if the ghost is in jail, put all the particles there set all the other positions to zero:
        if noisyDistance == None:
            ghostPosition = self.getJailPosition()
            temp = []
            for i in range (0, self.numParticles):
                temp.append(ghostPosition)
            self.particles = temp
            ...
            for particle in self.particles:
                ...
                allPossible[particle] = emissionModel[trueDistance]*beliefs[particle]
            allPossible.normalize()
    ...
    def elapseTime(self, gameState):
        ...
        #     #iterate over new Pos Dist:
        #     for newPos, prob in newPosDist.items():
        #         allPossible[newPos] = allPossible[newPos] + newPosDist[newPos] * beliefs[position]
        # allPossible.normalize()
    ...
    def getBeliefDistribution(self):
        ...
        #loop through position in self.legalpositions
        #for each one, add akey to the dictionary with a value of 0
        #loop through particles
        #for eachparticle, increment value
        #normalize
        #return beliefs
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        self.particles = []
        counter = self.numParticles
        self.permutations = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        random.shuffle(self.permutations)
        while counter > 0:
            #loop through permutations rather than just legal positions
            for i in range(0, len(self.permutations)):
                #append permutation
                self.particles.append(self.permutations[i])
                counter = counter - 1
    ...
    def observeState(self, gameState):
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        # check if a ghost is in jail and update particles accordingly
        captured = []
        for i in range(0, self.numGhosts):
            if noisyDistances[i] == None and i not in captured:
                newParticles = []
                for particle in self.particles:
                    newParticle = self.getParticleWithGhostInJail(particle, i)
        ...
        #update allpossible
        for particle in self.particles:
            #set weight to 1 initially so that you don't multiply by zero below
            ...
            for i in range(0, len(particleList)):
                if noisyDistances[i] == None:
                    ...
                    trueDistance = util.manhattanDistance(particleList[i], pacmanPosition)
                    allPossible[particle] *= emissionModels[i][trueDistance]
            allPossible[particle] *= beliefs[particle]
        allPossible.normalize()
    ...
    def elapseTime(self, gameState):
        ...
        **Note** that you may need to replace "prevGhostPositions" with the
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            for i in range(0, len(newParticle)):
        ...
        #previous elapseTime below for reference
        # newParticles = []
        # for particle in self.particles:
        #     newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, particle))
        #     newParticles.append(util.sample(newPosDist))
    ...
    def getBeliefDistribution(self):
        ...
        #update this to includepermutations rather than just legal positions
        for permutation in self.permutations:
            beliefs[permutation] = 0
