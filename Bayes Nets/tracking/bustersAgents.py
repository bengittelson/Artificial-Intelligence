...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        #get dictionary of max probability for each ghost
        for ghost in livingGhostPositionDistributions:
            maxProb = -1
            for position in ghost:
                curProb = ghost[position]
                if curProb > maxProb:
                    maxProb = curProb
                    maxPos = position
                #break ties using mazeDistance to current position
                # if curProb == maxProb:
                #     if self.distancer.getDistance(position, pacmanPosition) < self.distancer.getDistance(maxPos, pacmanPosition):
                #         maxProb = curProb
                #         maxPos = position
            probsList[maxPos] = maxProb

        import sys

        minDistance = sys.maxint
        minPos = None
        for pos in probsList:
            curDistance = self.distancer.getDistance(pos, pacmanPosition)
            if curDistance < minDistance:
                minDistance = curDistance
                minPos = pos

        #get the best action
        bestActionDist = sys.maxint
        bestAction = None
        for action in legal:
            successor = Actions.getSuccessor(pacmanPosition, action)
            #calculate distance from successor to nearest ghost
            successorDistance = self.distancer.getDistance(successor, minPos)
            if successorDistance < bestActionDist:
                bestActionDist = successorDistance
                bestAction = action
